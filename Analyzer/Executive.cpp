...
int main(int argc, char* argv[])
{
  ...
  try {
	...
	std::cout << "\n\n******<Requirement 3>Creted packages as TypeTable(Type Analysis), Strongly Connected Components, Dependency Analysis and Display packages***********";
	Display disp;	
	DependencyAnalysis d(exec.getFileMap());
	d.doDependAnal();
	NoSqlDb<std::string> database;
	database = d.getDatabase();
	std::cout << "\n\n************<Requirement 4> Type Table package that identifies all of the types defined in a specified file**************";
	disp.showTypeTable(d.getTheTable());
	std::cout << "\n\n************<End of Requirement 4 demonstration> **************";
	std::cout << "\n\n******<Requirement 5>Created Dependency Analysis package that identifies the dependencies between files***********";
	disp.showDependencyTable(database);	
	std::cout << "\n\n************<End of Requirement 5 demonstration> **************";
	std::cout << "\n\n******<Requirement 6>finding strongly connected component using the dependencies defined in the previous reuirement***********";
	SCC getSCC(database);
	getSCC.BuildTheGraph();
	getSCC.ComputeSCC();
	getSCC.persistsIntoXml();
	disp.showStronglyConnectedGraph(getSCC);
	std::cout << "\n\n************<End of Requirement 6 demonstration> **************";
  }
